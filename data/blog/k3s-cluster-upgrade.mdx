...
# k3s Upgrade with GiOps + System Upgrade Controller leveraging Codex
...
- Technically, zero-downtime upgrade of Postgres workloads was impossible, but can be mitigated in application level with proper retrying and connection pool settings.
- CNPG database failover in `dev` took about `11s` for the primary promotion window.
- With manual switchover using `kubectl cnpg promote ...`, primary switchover took less than `4s`.

## Table of Contents

1. Why This Process Was Designed This Way
2. Upgrade Plan Details
3. Execution Timeline (UTC, 2026-03-02)
4. Final Result
5. What I’d Keep for Future Upgrades
...
Also, I was curious to see how the AI-assisted upgrade design and execution would play out in a real cluster environment.

My cluster runs:

- 1 control-plane node + 4 worker nodes
- ArgoCD-managed applications (`dev`, `prod`) and observability stack including Prometheus, Grafana and Loki
...
This was my first in-place upgrade for Kubernetes cluster.  
...
But since this was my on-premise homelab cluster, blue-green was not an option.

I could have done a more manual process by sshing to each node and upgrading k3s directly, but that would have been more error-prone and less auditable.
...
- Pinned version in Git: `v1.34.4+k3s1` to avoid automatic upgrades when the new version is released
...
- Detailed measured impact: `docs/k3s-failover-impact-report.md`
...
Before touching node versions, I focused on availability posture:

- Added PDBs (`minAvailable: 1`) for my applications in dev, prod namespaces.
...
If a database pod is recreated on another node but still references the old PVC/PV, it cannot mount that volume because the data path physically exists on the previous node.
So even though CNPG does not use StatefulSet and uses its own CRDs, it still relies on Kubernetes PVC/PV primitives, and the same node-local storage limitation applies.

On a cluster with `local-path`, this prep is not optional.
Reference:
