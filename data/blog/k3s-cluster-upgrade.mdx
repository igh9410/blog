...
# K3s 업그레이드 회고: GitOps + SUC + Codex로 진행
...
- Postgres 워크로드를 완전 무중단으로 올리기는 현실적으로 어려웠지만, 앱 쪽 retry/connection pool 설정으로 체감 영향을 줄일 수 있었습니다.
- primary pod 삭제 기반 CNPG failover의 primary promotion 구간은 약 `11초`였고, `kubectl cnpg promote ...` 수동 switchover는 `4초 미만`이었습니다.
- Codex가 end-to-end로 대부분 처리해줬지만, 프롬프트 전에 CNPG 공식 문서를 먼저 봤으면 업그레이드 플로우를 더 잘 짤 수 있었겠다는 아쉬움이 있습니다.
...
- 앱 레벨에서 명확히 보인 중단은 `dev/gramnuri-api` failover 구간의 짧은 오류뿐이었습니다
